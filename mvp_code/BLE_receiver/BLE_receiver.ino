...
int active[15][3];            //[0] - id ; [1] - rssi; [2] to send update ?
unsigned long times[15];      //last seen in ms (from active)
...
void sendInfo(int type, int idx){
  ...
  if(type == NEAR){
    ...
    //Serial.print(active[idx][0], HEX);Serial.print(",  RSSI: ");Serial.println(active[idx][1]);
    ...
    //Serial.print("Beacon OUT : "); Serial.println(active[idx][0], HEX);
    Telemetry data[data_items] = {
      { "state", AWAY },
      { "id",    active[idx][0] },
      { "rssi",  0 },
    ...
    void onResult(BLEAdvertisedDevice advertisedDevice) {
      ...
      if(advertisedDevice.getManufacturerData().length() > 22){
        if(memcmp((uint8_t*)uuid_system, (uint8_t*)&advertisement[4],16) == 0){
          ...
          //Serial.print("Found! Beacon ID: "); Serial.println(beaconID, HEX);
          ...
          //Serial.print("RSSI: "); Serial.println(beaconRSSI);Serial.println();
        }
      }
    }
};
